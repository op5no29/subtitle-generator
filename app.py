...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("### 📋 法的情報")
        if st.button("📄 特定商取引法に基づく表記", use_container_width=True, key="sidebar_commerce"):
            st.session_state.page = "commerce_disclosure"
            st.rerun()
...
def render_commerce_disclosure():
    """特定商取引法に基づく表記ページ"""
    st.markdown("""
    <div style="max-width: 800px; margin: 0 auto;">
        <h1 style="color: #1f77b4; border-bottom: 3px solid #1f77b4; padding-bottom: 1rem;">
            📋 特定商取引法に基づく表記
        </h1>
    </div>
    """, unsafe_allow_html=True)
    ...
    disclosure_items = [
        ("事業者名", "島田誌音"),
        ("サービス名称", "動画・音声文字起こしアプリ（プロフェッショナル版）"),
        ("所在地", "〒000-0000 日本国内\n※個人事業者のため詳細住所は請求があった場合に遅滞なく開示いたします"),
        ("連絡先", "Twitter: @c_y_l_i (https://x.com/c_y_l_i)\n※お問い合わせはTwitterのDMにてお願いいたします"),
        ("販売価格", "プレミアムプラン: 月額500円（税込）\n※クレジットカード決済（Stripe経由）"),
        ("商品・サービス内容", """AI技術を活用した以下のサービス：
• 動画ファイルからの自動字幕生成・焼き込み
• 音声ファイルの高精度文字起こし  
• 多言語翻訳機能（日英中韓対応）
• リアルタイム録音・文字起こし機能
• 処理結果のダウンロード機能"""),
        ("お支払い方法", "クレジットカード決済（Stripe決済システム利用）\n対応カード: Visa、MasterCard、American Express、JCB等"),
        ("お支払い時期", "月額課金制（毎月同日に自動決済）\n初回決済: 登録完了時\n継続決済: 毎月の登録日に自動実行"),
        ("サービス提供時期", "決済完了後、即座にサービス利用可能\n24時間365日いつでもご利用いただけます"),
        ("返品・キャンセルについて", """デジタルサービスの性質上、サービス提供開始後の返品・返金は原則として承っておりません。
ただし、以下の場合は個別に対応いたします：
• システム障害によりサービスが利用できない場合
• 当方の責任による重大な不具合が発生した場合

解約について：
解約はいつでも可能です。解約した月の末日まではサービスをご利用いただけます。"""),
        ("利用規約・プライバシーポリシー", """サービス利用前に必ずご確認ください：
• アップロードされたファイルは処理後24時間以内に自動削除
• 個人情報は適切に管理し、第三者に提供いたしません
• OpenAI Whisper API、Anthropic Claude APIを利用
• 違法なコンテンツの処理は禁止""")
    ]
    
    for title, content in disclosure_items:
        with st.container():
            st.markdown(f"""
            <div style="
            ...
            ">
                ...
                <div style="margin-left: 1rem; white-space: pre-line;">
                    {content}
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    ">
        <h3>🔰 サポート・お問い合わせ</h3>
        <p>
            技術的なお問い合わせやサポートが必要な場合は、以下までご連絡ください：<br>
            <strong>Twitter:</strong> <a href="https://x.com/c_y_l_i" target="_blank">@c_y_l_i</a><br>
            <strong>営業時間:</strong> 平日 9:00-18:00（土日祝日は返信が遅れる場合があります）
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    ">
        ...
        本サービスは個人事業として運営されており、大学生向けの学習支援ツールとして提供しています。
        サービス内容や料金に関してご不明な点がございましたら、購入前に必ずお問い合わせください。
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="text-align: center; margin-top: 2rem; color: #666; font-size: 0.9rem;">
        <p>最終更新日: 2025年8月9日</p>
        <p>&copy; 2025 島田誌音 - 動画・音声文字起こしアプリ</p>
    </div>
    """, unsafe_allow_html=True)
    else:
        # ログイン済み状態
        ...
        elif st.session_state.page == "commerce_disclosure":
            render_commerce_disclosure()
        else:
            # メインアプリ
            display_header()
    ...
    <div style="text-align: center; color: #666; padding: 1rem 0;">
        ...
        <p><small>
            ...
            <a href="#" onclick="window.location.reload(); document.querySelector('button[data-testid=sidebar-commerce]').click(); return false;" 
